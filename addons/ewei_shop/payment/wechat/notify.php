            ...
            if (empty($type)) {
                ...
                if (!empty($log) && empty($log["status"]) && $log["openid"] == $get["openid"] && $log["money"] == $total_fee) {
                    ...
                    m("member")->setCredit($log["openid"], "credit2", $log["money"], array(
                        0,
                        "大数据商城会员充值:wechatnotify:credit2:" . $log["money"]
                    ));
                ...
                if (!empty($log) && empty($log["status"]) && $log["openid"] == $get["openid"]) {
                ...
                if (!empty($log) && empty($log["dispatchstatus"]) && $log["openid"] == $get["openid"]) {
                    pdo_update("ewei_shop_creditshop_log", array(
                        "dispatchstatus" => 1
                    ), array(
                        "id" => $log["id"]
