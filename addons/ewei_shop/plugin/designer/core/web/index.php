...
if ($op == 'display') {
    ...
    if (!empty($pageid)) {
        ...
        if (strexists($datas['datas'], '{')) {
            $data = htmlspecialchars_decode($datas['datas']);
        } else {
            $data = htmlspecialchars_decode(base64_decode($datas['datas']));
        }
        ...
        $data     = rtrim($data, "]");
        $data     = ltrim($data, "[");
        if (strexists($datas['pageinfo'], '{')) {
            $pageinfo = htmlspecialchars_decode($datas['pageinfo']);
        } else {
            $pageinfo = htmlspecialchars_decode(base64_decode($datas['pageinfo']));
        }
    ...
    if ($_W['ispost']) {
        if ($apido == 'savepage') {
            $id                    = $_GPC['pageid'];
            $datas                 = json_decode(htmlspecialchars_decode($_POST["datas"]), true);
            ...
            foreach ($datas as &$data) {
                if ($data['temp'] == 'banner' || $data['temp'] == 'menu' || $data['temp'] == 'picture') {
                    ...
                    unset($d);
                    $data["params"]["goodhref"] = $this->createMobileUrl("shop/detail");
            ...
            $insert = array(
                'pagename' => $pagename,
                'pagetype' => $pagetype,
                'pageinfo' => base64_encode(json_encode($p)),
                'savetime' => $date,
