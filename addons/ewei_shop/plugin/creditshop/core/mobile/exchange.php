...
$id        = intval($_GPC['id']);
if ($operation == 'check') {
    $log = pdo_fetch("select id,status from " . tablename('ewei_shop_creditshop_log') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(
        ':id' => $id,
        ':uniacid' => $uniacid,
        ':openid' => $openid
    ));
    ...
    $log = pdo_fetch("select id,eno from " . tablename('ewei_shop_creditshop_log') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(
        ':id' => $id,
        ':uniacid' => $uniacid,
        ':openid' => $openid
    ));
