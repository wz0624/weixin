...
if ($operation == 'display') {
    ...
    if (!empty($_GPC['displayorder'])) {
        ...
        foreach ($_GPC['displayorder'] as $id => $displayorder) {
            pdo_update('ewei_shop_creditshop_adv', array(
        ...
        message('分类排序更新成功！', $this->createPluginWebUrl('creditshop/adv', array(
            'op' => 'display'
        )), 'success');
    }
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('ewei_shop_creditshop_adv', $data, array(
                'id' => $id
            ));
        ...
        message('更新幻灯片成功！', $this->createPluginWebUrl('creditshop/adv', array(
            'op' => 'display'
        )), 'success');
    }
    $item = pdo_fetch("select * from " . tablename('ewei_shop_creditshop_adv') . " where id=:id and uniacid=:uniacid limit 1", array(
        ":id" => $id,
        ":uniacid" => $_W['uniacid']
    ));
    ...
    if (empty($item)) {
        message('抱歉，幻灯片不存在或是已经被删除！', $this->createPluginWebUrl('creditshop/adv', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('ewei_shop_creditshop_adv', array(
        'id' => $id
    ));
    plog('creditshop.adv.delete', "删除积分商城幻灯片 ID: {$id} 标题: {$item['advname']} ");
    message('幻灯片删除成功！', $this->createPluginWebUrl('creditshop/adv', array(
        'op' => 'display'
    )), 'success');
}
load()->func('tpl');
