...
if ($operation == 'display') {
    ...
    if (!empty($_GPC['displayorder'])) {
        ...
        foreach ($_GPC['displayorder'] as $id => $displayorder) {
            pdo_update('ewei_shop_creditshop_category', array(
        ...
        message('分类排序更新成功！', $this->createPluginWebUrl('creditshop/category', array(
            'op' => 'display'
        )), 'success');
    }
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('ewei_shop_creditshop_category', $data, array(
                'id' => $id
            ));
        ...
        message('更新分类成功！', $this->createPluginWebUrl('creditshop/category', array(
            'op' => 'display'
        )), 'success');
    }
    $item = pdo_fetch("select * from " . tablename('ewei_shop_creditshop_category') . " where id=:id and uniacid=:uniacid limit 1", array(
        ":id" => $id,
        ":uniacid" => $_W['uniacid']
    ));
    ...
    if (empty($item)) {
        message('抱歉，分类不存在或是已经被删除！', $this->createPluginWebUrl('creditshop/category', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('ewei_shop_creditshop_category', array(
        'id' => $id
    ));
    plog('creditshop.category.delete', "删除积分商城分类 ID: {$id} 标题: {$item['name']} ");
    message('分类删除成功！', $this->createPluginWebUrl('creditshop/category', array(
        'op' => 'display'
    )), 'success');
}
load()->func('tpl');
