...
if ($operation == 'display') {
	...
	$mem = pdo_fetch("select * from " . tablename('ewei_shop_member') . " where id=:uniacid  limit 1 ", array(
        ':uniacid' => $id
        
    ));
	$memb = pdo_fetch("select * from " . tablename('ewei_shop_member') . " where id=:uniacid  limit 1 ", array(
        ':uniacid' => $mem['agentid']
