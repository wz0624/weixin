...
if ($operation == 'display') {
    ...
    $roles = pdo_fetchall('select id,rolename from ' . tablename('ewei_shop_perm_role') . ' where uniacid=:uniacid and deleted=0', array(
        ':uniacid' => $_W['uniacid']
    ));
    ...
    $item       = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_perm_user') . " WHERE id =:id and deleted=0 and uniacid=:uniacid limit 1", array(
        ':uniacid' => $_W['uniacid'],
        ':id' => $id
    ));
    ...
    if (!empty($item)) {
        ...
        $role = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_perm_role') . " WHERE id =:id and deleted=0 and uniacid=:uniacid limit 1", array(
            ':uniacid' => $_W['uniacid'],
            ':id' => $item['roleid']
        ));
        if (!empty($role)) {
            ...
            if (user_check(array(
                ...
                if (!user_check(array(
                    ...
                    die(json_encode(array(
                        'result' => 0,
                        'message' => '此用户为系统存在用户，但是您输入的密码不正确，无法添加'
                    )));
                }
                $user             = user_single(array(
                    'username' => $item['username']
    ...
    if (empty($item)) {
        message('抱歉，操作员不存在或是已经被删除！', $this->createPluginWebUrl('perm/user', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('ewei_shop_perm_user', array(
        'id' => $id,
        'uniacid' => $_W['uniacid']
    ));
    plog('perm.user.delete', "删除操作员 ID: {$id} 用户名: {$item['username']} ");
    message('操作员删除成功！', $this->createPluginWebUrl('perm/user', array(
        'op' => 'display'
    )), 'success');
}
load()->func('tpl');
