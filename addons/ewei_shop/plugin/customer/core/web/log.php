...
if ($operation == "display") {
    ...
    if (!empty($log["addressid"])) {
        $address = pdo_fetch("select id,realname,mobile,address,province,city,area from " . tablename("ewei_shop_member_address") . " where id=:id and uniacid=:u
niacid limit 1", array(
            ":id" => $log["addressid"],
            ":uniacid" => $_W["uniacid"]
        ));
    }
