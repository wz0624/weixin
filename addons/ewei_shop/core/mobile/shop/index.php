...
$set = set_medias(m("common")->getSysset("shop"), array(
    "logo",
    "img"
));
if ($_W["isajax"]) {
    if ($operation == "index") {
        ...
        $category = pdo_fetchall("select id,name,thumb,parentid,level from " . tablename("ewei_shop_category") . " where uniacid=:uniacid and ishome=1 and enabled=1 order by displayorder desc", array(
            ":uniacid" => $uniacid
        ));
        ...
        foreach ($category as &$c) {
            ...
            if ($c["level"] == 3) {
                $c["url"] = $this->createMobileUrl("shop/list", array(
                    "tcate" => $c["id"]
                ));
            } else if ($c["level"] == 2) {
                $c["url"] = $this->createMobileUrl("shop/list", array(
