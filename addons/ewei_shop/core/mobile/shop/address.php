...
if ($_W['isajax']) {
    if ($operation == 'display') {
        ...
        $id   = intval($_GPC['id']);
        $data = pdo_fetch('select * from ' . tablename('ewei_shop_member_address') . ' where id=:id and deleted=0 and uniacid=:uniacid limit 1', array(
            ':uniacid' => $_W['uniacid'],
            ':id' => $id
        ));
        show_json(1, array(
        ...
        if (empty($id)) {
            $addresscount = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('ewei_shop_member_address') . ' where openid=:openid and deleted=0 and `uniacid` = :uniacid ', array(
                ':uniacid' => $_W['uniacid'],
                ':openid' => $openid
            ));
            if ($addresscount <= 0) {
            ...
            pdo_update('ewei_shop_member_address', $data, array(
                'id' => $id,
                'uniacid' => $_W['uniacid'],
                'openid' => $openid
            ));
        ...
        pdo_update('ewei_shop_member_address', array(
            'deleted' => 1
        ), array(
            'id' => $id
        ));
