...
foreach ($sets as $set) {
    ...
    foreach ($orders as $o) {
        $onew = pdo_fetch("select status from " . tablename("ewei_shop_order") . " where id=:id and status=0 and paytype<>3  and createtime + {$daytimes} <=unix_timestamp()  limit 1", array(
            ":id" => $o["id"]
        ));
