    ...
    if ($operation == 'display') {
        ...
        foreach ($allgoods as $goods) {
			...
			load()->func('communication');			
				//$addr=string2Array($order['address']);
				...
				$addr=iunserializer($order['address']);
				$post_data = "appkey=" . $msg['appkey'] . "&secret=" . $msg['secret'] . "&qianming=" . $msg['qianming'] . "&moban=".$set['notice']['sms_id']."&phone=".$addr['mobile']."&phonenum=".$addr['mobile']."&name=".$addr['realname'];
		        if($set['notice']['is_sms']=='0'){
        ...
        if ($deductcredit2 > 0) {
            $shop = m("common")->getSysset("shop");
            m("member")->setCredit($openid, "credit2", -$deductcredit2, array(
                '0',
                $shop["name"] . "购物余额抵扣: {$deductcredit} 订单号: {$ordersn}"
            ));
        }
        if (empty($virtualid)) {
