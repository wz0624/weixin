 ...
 */
	...
	if ($_W['isajax']) {
			...
			$rows = pdo_fetchall("SELECT * FROM".tablename('xcommunity_goods')."WHERE weid=:weid AND isrecommand =1 AND status = 1 AND type = 1 AND " . time() . ">=starttime and " . time() . "<=endtime LIMIT ".($pindex - 1) * $psize.','.$psize,array(':weid' => $_W['uniacid']));
