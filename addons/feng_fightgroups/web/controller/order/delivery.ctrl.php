...
if($op == 'display'){
	$template = pdo_fetchall("select * from".tablename('tg_delivery_template')." WHERE uniacid = '{$_W['uniacid']}'");
	$prices = array();
...
if ($op=='post') {
	...
	$merchants = pdo_fetchall("SELECT * FROM " . tablename('tg_merchant') . " WHERE uniacid = '{$_W['uniacid']}'");
