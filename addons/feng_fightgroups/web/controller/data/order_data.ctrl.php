<?php
	...
	if($op=='display'){
		...
		$payorders = pdo_fetchall("select distinct(openid) from".tablename('tg_order')."where uniacid={$_W['uniacid']} and status in(1,2,3,4,6,7)");
		$payordersnum = pdo_fetchall("select id from".tablename('tg_order')."where uniacid={$_W['uniacid']} and status in(1,2,3,4,6,7)");
		$allorders2 = pdo_fetchall("select pay_price from".tablename('tg_order')."where uniacid={$_W['uniacid']} and status in(1,2,3,4,6,7) ");
