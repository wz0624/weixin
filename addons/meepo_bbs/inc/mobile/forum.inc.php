<?php
global $_W,$_GPC;
include MODLE_ROOT.'inc/core/mobile/_init.php';
checkBbsUser();
$table = 'meepo_bbs_set';
...
$navs = getnavs();
//检查签到
// $sql = "SELECT id FROM ".tablename('meepo_bbs_sign')." WHERE uniacid = :uniacid AND uid = :uid";
// $issign = pdo_fetchcolumn($sql,$params);
...
$where = " AND 1 ";
if($forum['post_topics']){
	$where .= " AND status = :status";
	$params[':status'] = 1;
}
if($id){
	$return = check_look($id);
	if(is_error($return)){
...
if(!empty($_GPC['srchtxt'])){
	$srchtxt = trim($_GPC['srchtxt']);
