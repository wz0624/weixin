...
if($_W['ispost']){
	...
	if(!empty($_GPC['uploadall'])){
		$in = db_create_in($select,'b.id');
		$sql = "SELECT b.*,m.avatar,m.nickname FROM ".tablename('meepo_bbs_topics')." as b LEFT JOIN ".tablename('mc_members')
				." AS m ON b.uid = m.uid WHERE b.uniacid = '{$_W['uniacid']}'";
		$params = array();
		$list = pdo_fetchall($sql,$params);
		//导出
		include_once ('../framework/library/phpexcel/PHPExcel.php');
...
if(empty($fo)){
	...
	foreach ($lists as $li) {
		...
		if($li['status'] == 0){
			$li['status_label'] = 'warning';
			$li['status']  = '待审核';
		}else{
			$li['status_label'] = 'success';
			$li['status']  = '通过';
		}
		$li['status_button'] = $this->createWebUrl('manage',array('fo'=>'status','id'=>$li['id']));
		...
		$li['commont'] = $this->createWebUrl('manage',array('fo'=>'commont','tid'=>$li['id']));
...
if($fo == 'commont'){
	...
	if($_W['ispost']){
		...
		message('添加评论成功',referer(),success);
	}
	
	include $this->template('manage_topic_reply');
}
...
if($fo  == 'status'){
	...
	pdo_update('meepo_bbs_topics',array('status'=>$status),array('id'=>$id));
