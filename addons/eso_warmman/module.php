...
class Eso_WarmmanModule extends WeModule {
	public function fieldsFormDisplay($rid = 0) {
		//要嵌入规则编辑页的自定义内容，这里 $rid 为对应的规则编号，新增时为 0
		global $_W;
		load()->func('tpl');
		$reply = pdo_fetch("SELECT * FROM ".tablename('eso_warmman_reply')." WHERE rid = :rid", array(':rid' => $rid));
		if (empty($reply)) {
	...
	public function fieldsFormSubmit($rid) {
		...
		if (empty($reid)) {
			pdo_insert('eso_warmman_reply', $data);
		} else {
			pdo_update('eso_warmman_reply', $data, array('id' => $reid));
		}
	}

	public function ruleDeleted($rid) {
		//删除规则时调用，这里 $rid 为对应的规则
		pdo_delete('eso_warmman_reply', array('rid' => $rid));
		pdo_delete('eso_warmman_users', array('rid' => $rid));
		pdo_delete('eso_warmman_submit', array('rid' => $rid));
	}
}