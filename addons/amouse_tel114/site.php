...
define("AMOUSE_TEL114", "amouse_tel114");
define("AMOUSE_RES", "../addons/".AMOUSE_TEL114."/style/");
require_once IA_ROOT .'/addons/amouse_tel114/plugin/reader.php';
    ...
    public function doWebTel() {
        ...
        if ($op == 'display') {
            ...
            if (checksubmit('submit')) {
                ...
                $flag = $this->checkUploadFileMIME($_FILES['file']);
                if ($flag == 0) {
                    message('文件格式不对.');
                }
                if (empty($tmp)) {
                ...
                $filePath = IA_ROOT .'/addons/amouse_tel114/upFile/';
                $extend = strrchr($filename, '.');
                //上传后的文件名
                $file_name=$filePath.date('Ymdhis').$extend;
                if (copy($tmp, $file_name)) {
                    ...
                    unlink($file_name);
                    message('成功导入'.$total.'条！', $this->createWebUrl('tel', array('op'=>'display')), 'success');
                }
            }
        }
