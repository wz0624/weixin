...
$weid = $_W['uniacid'];
$openid = $_W['openid'];
$ss_status = intval($_GPC['ss_status']);
...
if($ss_status==0){
	$lists = pdo_fetchall("SELECT * FROM " . $tablename . " WHERE weid = :weid AND nickname like '%{$keword}%' AND isshow=:isshow AND yingcang=:yingcang   AND tuijian=:tuijian AND tj_over_time>=:tj_over_time ORDER BY love DESC,id DESC,time DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(':weid' =>$weid,':isshow'=>'1',':yingcang'=>'1',':tuijian'=>'2',':tj_over_time'=>time()));
	if(!empty($lists)){
		...
		if(count($lists)==5){
			...
			$lists = pdo_fetchall("SELECT * FROM " . $tablename . " WHERE weid = :weid AND nickname like '%{$keword}%'  AND isshow=:isshow AND yingcang=:yingcang   AND tj_over_time<:tj_over_time ORDER BY love DESC,id DESC,time DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(':weid' =>$weid,':isshow'=>'1',':yingcang'=>'1',':tj_over_time'=>TIMESTAMP));
			if(!empty($lists)){
						foreach($lists as $row){
								if(!empty($row['lat']) && !empty($row['lng'])){
									if(!empty($julires['lat']) && !empty($julires['lng'])){
