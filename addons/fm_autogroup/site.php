...
class Fm_autogroupModuleSite extends WeModuleSite {
	...
	public function doWebsendMobileRegMsg() {
		global $_GPC,$_W;
		$groupid = $_GPC['gid'];
		$id = $_GPC['id'];
		$url = $_GPC['url'];
		...
		if ($groupid == -1) {
			...
			$g = pdo_fetch("SELECT * FROM ".tablename('fm_autogroup_group')." WHERE uniacid = '{$_W['uniacid']}' $w");		
			$uw = " AND gname = '{$g['gname']}'";
		}
		...
		$userinfo = pdo_fetchall("SELECT * FROM ".tablename('fm_autogroup_members')." WHERE uniacid = '{$_W['uniacid']}' $uw ORDER BY gname ASC, id DESC LIMIT ".($pindex - 1) * $psize.','.$psize);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('fm_autogroup_members') . " WHERE uniacid = '{$_W['uniacid']}' $uw");
		$pager = pagination($total, $pindex, $psize);
		...
		foreach ($userinfo as $mid => $u) {
			...
			include 'mtemplate/site.php';		
			if (!empty($template_id)) {
				$this->sendtempmsg($template_id, $url, $data, '#FF0000', $u['from_user']);
			}
			if (($psize-1) == $mid) {
				...
				$page = $pindex + 1;
				$to = $this->createWebUrl('sendMobileRegMsg', array('gid' => $groupid,'id' => $id,'url' => $url, 'page' => $page));
				message($msg, $to);
			}
		}
		
		message('发送成功！', url('site/article/display'));
	}
