...
class Aki_yzmjfModuleProcessor extends WeModuleProcessor
{
    public function respond()
    {
        ...
        $res     = pdo_fetch("select count(*) as count,id,yzmjfid,piciid,jifen  from " . tablename("aki_yzmjf_code") . " where uniacid = :uniacid and  code = :code and status = 1  and yzmjfid = 0", array(
            ':code' => $code,
            ':uniacid' => $_W['uniacid']
        ));
        $count   = $res['count'];
