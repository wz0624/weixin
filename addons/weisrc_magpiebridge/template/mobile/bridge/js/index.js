window.onload = function() {
    ...
    btnSubmit.onclick = function(e) {
        var nick = document.querySelector("[name=username]");
    ...
    btnStart.onclick = function() {
        if (window.start_page_config.ISEND) {
            panelBox2.querySelector(".plus").innerHTML = "该活动已结束！";
            panelBox2.classList.add("show");
            panelLayer.classList.add("show");
            return;
        } else if (!window.start_page_config.NEEDREGISTER) {
    ...
    function calTableHeight(removeTop) {
        ...
        var num = bodyHeight - (containerTitle.offsetHeight + tableTitleHeight + tableTitleTop + tableBottomHeight + domAd.offsetHeight + companyRightsHeight + minus);
