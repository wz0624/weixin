...
class Welian_Indiana_Codes{
	...
	public function code($openid='',$tid = '',$uniacid=0,$machine_time = ''){
		...
		if($machine_time != ''){
			$consumerecord_time = $machine_time;		//机器人时间判定
		}else{
		...
		foreach ($myCart as $key => $value) {
			...
			$result_period = pdo_fetch("select id,goodsid,shengyu_codes,period_number from ".tablename('weliam_indiana_period')." where uniacid = :uniacid and period_number = :period_number",$data_period);
			...
			m('credit')->updateCredit2($value['openid'],$_W['uniacid'],-$allmoney);
			self::deleteCart($value['id']);		//清除购物车中商品记录
			
			if($result_period['shengyu_codes'] == 0){
				//本期商品结束进入开奖阶段
				...
				if($result_goodslist['periods'] < $result_goodslist['maxperiods']){
					...
					if($result_periods_one['shengyu_codes'] == 0){
	...
	public function create_newgoods($goodsid = ''){
		...
		if($goodsid != ''){
			//创建新的一期商品
			$select_goodslist = " canyurenshu,id,maxperiods,periods,price,jiexiao_time,couponid,uniacid,init_money,next_init_money ";
	...
	/***************计算夺宝码****************/
	public function create_code($period_number = ''){
		global $_W;
